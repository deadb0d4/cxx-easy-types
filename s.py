...
def create_types(namespace, name, desc):
    ...
    if 'enum' in struct:
        res = EnumClass(
            name, namespace.name, struct['enum'])
    ...
    if type(struct) is list:
        ...
        res = Array(name, namespace.name, create_types(
            namespace, f'{name}Item', struct[0]))
    namespace.types[res.name] = res
    return res
...
enum_string_util_hpp_template = open_template(
    'templates/enum_string.hpp.jinja')
enum_string_util_cpp_template = open_template(
    'templates/enum_string.cpp.jinja')
